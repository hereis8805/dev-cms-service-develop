...
function Search(props) {
  ...
  function onSubmit(values) {
    setRequestParam({
      ...requestParam,
      settleManageCp: { ...values, selectDate: requestParam?.settleManageCp?.selectDate }
    });
  }
          ...
          <Grid container direction="row" justifyContent="space-between" alignItems="baseline">
            ...
            <Grid item>
                ...
                onClick={() => {
                  setRequestParam({
                    ...
                    settleManageCp: {
                      ...requestParam?.settleManageCp,
                      selectDate: format(
                        add(parse(requestParam?.settleManageCp?.selectDate, "yyyy-MM", new Date()), { months: -1 }),
                        "yyyy-MM"
                      )
                    }
                  });
              ...
              <DatePicker
                ...
                openTo="month"
                format="yyyy-MM"
                value={requestParam?.settleManageCp?.selectDate}
                onChange={(nextDate) => {
                  setRequestParam({
                    ...requestParam,
                    settleManageCp: {
