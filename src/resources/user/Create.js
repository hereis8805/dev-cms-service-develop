...
import { isSignUp } from "axios/user/user";
...
function UserCreate(props) {
  ...
  const [guPassword, setGuPassword] = useState("");
  ...
  const handleSetIsSignUp = async (param) => {
    await isSignUp(param);
  };
                ...
                onClick={() => {
                  if (window.confirm("정말 등록하시겠습니까?")) {
                    const param = {
                      GU_GRADE: guGrade,
                      GU_USER_ID: guUserId,
                      GU_PASSWORD: guPassword,
                      GU_NAME: guName,
                      GU_EMAIL: guEmail,
                      GU_DESCRIPTION: guDescription,
                      GU_CP_ID: gpCpId
                    };
                    handleSetIsSignUp(param);
                    alert("등록완료");
