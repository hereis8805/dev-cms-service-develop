...
import { getPrepaidPaymentDetail, putPrepaidPaymentDetail } from "axios/costManagement/prepaidPayment/prepaidPayment";
...
function PrepaidPaymentManagementDetail(props) {
  ...
  const handlePutPrepaidPaymentDetail = async (param) => {
    await putPrepaidPaymentDetail(param);
  };
  ...
  const handleUnearnedRevenue = async (gpIdx) => {
    const prepaidDetail = await getPrepaidPaymentDetail(gpIdx);
    setPrepaidDetailInfo(prepaidDetail.data);
    setGpIdx(prepaidDetail.data.GP_IDX);
    setGpPrepaidType(prepaidDetail.data.GP_PREPAID_TYPE);
    setGpPrepaidSeq(prepaidDetail.data.GP_PREPAID_SEQ);
    setGpName(prepaidDetail.data.GP_NAME);
    setGpCpId(prepaidDetail.data.GP_CP_ID);
    setGpCpGroupId(prepaidDetail.data.GP_CP_GROUP);
    setGpCpName(prepaidDetail.data.GP_CP_NAME);
    setGpPriority(prepaidDetail.data.GP_PRIORITY);
    setGpSettlementType(prepaidDetail.data.GP_SETTLEMENT_TYPE);
    setGpStatus(prepaidDetail.data.GP_STATUS);
    setGcmManageId(prepaidDetail.data.GPD_MANAGE_ID);
    setGcmManageGroupId(prepaidDetail.data.GPD_MANAGE_GROUP);
    setPlatformDetail(prepaidDetail.data.GPD_PLATFORM);
    setPlatformGroup(prepaidDetail.data.GPD_PLATFORM_GROUP);
    setGpdSaleType(prepaidDetail.data.GPD_SALE_TYPE);
    setGpdSettlementRate(prepaidDetail.data.GPD_SETTLEMENT_RATE);
    setGpdSettlementPriority(prepaidDetail.data.GPD_SETTLEMENT_PRIORITY);
  };
  ...
  return (
    ...
    >
          ...
          <Button
            ...
            onClick={() => {
              if (window.confirm("정말 수정하시겠습니까?")) {
                ...
                handlePutPrepaidPaymentDetail(param);
                alert("수정완료");
                history.replace(`/prepaid`);
              }
            }}
      ...
      />
      <Div>
        <h3>{"선급금 관리 > 상세 - 수정내역"}</h3>
      </Div>
      <HistoryDetailList menuName={"선급금상세_수정"} ghRefIdx={props.match.params.id} />
    </Box>
  );
}

export default PrepaidPaymentManagementDetail;
