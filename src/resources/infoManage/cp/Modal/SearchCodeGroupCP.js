...
import { getCpPopupGroupCps } from "axios/information/cp";
...
function SearchCodeGroupCP(props) {
  ...
  // getCpPopupGroupCps
  const { isLoading, refetch } = useQuery(["getCpPopupGroupCps"], () => getCpPopupGroupCps(queryOptions), {
    enabled: false,
    onSuccess(data) {
      setresultData(
            ...
            <Grid item>
              <Button
                ...
                onClick={() => {
                  ...
                  const sumItem = [...popupList, ...result].map((item, index) => {
                    return {
                      ...item,
                      id: index + 1
                    };
...
export default SearchCodeGroupCP;
